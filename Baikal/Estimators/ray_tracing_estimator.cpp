#include "ray_tracing_estimator.h"
...
namespace Baikal
{
    static std::uint32_t constexpr kMaxSamplesPerBounce = 16u;

    struct RayTracingEstimator::PathState
    {
        float4 throughput;
        int volume;
        int flags;
    ...
    struct RayTracingEstimator::RenderData
    {
        // OpenCL stuff
    ...
    RayTracingEstimator::RayTracingEstimator(
        CLWContext context,
        RadeonRays::IntersectionApi* api
    ) :
        ClwClass(context, "../Baikal/Kernels/CL/ray_tracing_estimator.cl")
        , Estimator(api)
        , m_sample_counter(0)
    ...
    RayTracingEstimator::~RayTracingEstimator() = default;

    std::size_t RayTracingEstimator::GetWorkBufferSize() const
    {
        return m_render_data->rays[0].GetElementCount();
    }

    void RayTracingEstimator::SetWorkBufferSize(std::size_t s)
    {
        auto size = kMaxSamplesPerBounce * kMaxSamplesPerBounce * s;
    ...
    CLWBuffer<ray> RayTracingEstimator::GetRayBuffer() const
    {
        return m_render_data->rays[0];
    }

    CLWBuffer<int> RayTracingEstimator::GetOutputIndexBuffer() const
    {
        return m_render_data->output_indices;
    }

    CLWBuffer<int> RayTracingEstimator::GetRayCountBuffer() const
    {
        return m_render_data->hitcount;
    }

    void RayTracingEstimator::Estimate(
    ...
    void RayTracingEstimator::InitPathData(std::size_t size)
    {
    ...
    void RayTracingEstimator::ShadeSurface(
        ClwScene const& scene,
        int pass,
        std::size_t size,
        CLWBuffer<RadeonRays::float3> output,
    ...
    void RayTracingEstimator::ShadeVolume(
        ClwScene const& scene,
        int pass,
        std::size_t size,
        CLWBuffer<RadeonRays::float3> output,
    ...
    void RayTracingEstimator::EvaluateVolume(
        ClwScene const& scene,
        int pass,
        std::size_t size,
        CLWBuffer<RadeonRays::float3> output,
    ...
    void RayTracingEstimator::ShadeBackground(
        ClwScene const& scene,
        int pass,
        std::size_t size,
        CLWBuffer<RadeonRays::float3> output,
    ...
    void RayTracingEstimator::GatherLightSamples(
        ClwScene const& scene,
        int pass,
        std::size_t size,
        CLWBuffer<RadeonRays::float3> output,
    ...
    void RayTracingEstimator::RestorePixelIndices(int pass, std::size_t size)
    {
        // Fetch kernel
    ...
    void RayTracingEstimator::FilterPathStream(int pass, std::size_t size)
    {
    ...
    void RayTracingEstimator::ShadeMiss(
        ClwScene const& scene,
        int pass,
        std::size_t size,
        CLWBuffer<RadeonRays::float3> output,
    ...
    void RayTracingEstimator::SetRandomSeed(std::uint32_t seed)
    {
        std::srand(seed);
    }

    bool RayTracingEstimator::HasRandomBuffer(RandomBufferType buffer) const
    {
        switch (buffer)
        {
    ...
    CLWBuffer<std::uint32_t> RayTracingEstimator::GetRandomBuffer(RandomBufferType buffer) const
    {
        switch (buffer)
        {
        case RandomBufferType::kRandomSeed:
    ...
    CLWBuffer<RadeonRays::Intersection> RayTracingEstimator::GetFirstHitBuffer() const
    {
        return m_render_data->intersections;
    }

    void RayTracingEstimator::TraceFirstHit(
        ClwScene const& scene,
        std::size_t num_estimates
    )
    {
    ...
    void RayTracingEstimator::Benchmark(
        ClwScene const& scene,
        std::size_t num_estimates,
